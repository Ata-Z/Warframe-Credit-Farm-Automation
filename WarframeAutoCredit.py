...
# Global variable to control the macro's state
macro_running = False

# Coordinates and color for the operator form check
x, y = 1855, 958  # Screen coordinates
operator_color = (230, 229, 229)  # RGB color in operator form
...
def run_macro():
    # Run the main macro loop.
    while macro_running:
        # Press '5' to switch to operator form
        keyboard.press('5')
        time.sleep(random.uniform(0.05, 0.1))
        keyboard.release('5')
        time.sleep(random.uniform(0.5, 0.7))  # Small delay before pressing '5' again
        ...
        # Wait for a random time before restarting the loop
        ...
        if not check_operator_form():
            ...
            keyboard.press('5')  # Press '5' again to correct
            time.sleep(random.uniform(0.05, 0.1))
            keyboard.release('5')

def toggle_macro():
    # Toggle the macro on or off
